...
price_outlook<-price_outlook %>% fill(Header)%>%
  mutate(across(-c(1,2,Header), as.numeric))
...
#process non_opec_supply
non_opec_supply_data<-read_excel(path=excel_file,sheet = "3btab",range = "A5:BV36",na=c("n/a","-"))
...
nat_gas_data<-read_excel(path=excel_file,sheet = "5atab",range = "A4:BV38",na=c("n/a","-"))
names(nat_gas_data)<-c("code","Region",format(seq.Date(from=ymd(paste(year_start,month_start,1,sep="-")),by="1 month",length.out=72)))

nat_gas_data<-nat_gas_data%>% mutate(Header=ifelse(is.na(code),.[[2]],NA))%>%
  fill(Header,.direction=c("down"))%>%
  filter(!is.na(code))%>%
  relocate(Header,.after=2)%>%
  mutate(across(4:last_col(), ~ as.numeric(.)))%>%
  pivot_longer(cols=-c("code","Region","Header"),names_to = "Date",values_to ="value")%>%
  mutate(table="5atab",Date=ymd(Date),
  forecast=ifelse(Date>=file_date,1,0),
  version=file_date)%>%
    I()
...
# Power Gen
power_data<-NULL
if(year(file_date)>=2023)
{
power_data<-read_excel(path=excel_file,sheet = "7d(1)tab",range = "A4:BV65",na=c("n/a","-"))%>%
  bind_rows(read_excel(path=excel_file,sheet = "7d(2)tab",range = "A4:BV58",na=c("n/a","-")))
names(power_data)<-c("code","Header",format(seq.Date(from=ymd(paste(year_start,month_start,1,sep="-")),by="1 month",length.out=72)))

power_data<-power_data%>% mutate(Region=ifelse(is.na(code),.[[2]],NA))%>%
  fill(Region,.direction=c("down"))%>%
  filter(!is.na(code))%>%
  relocate(Region,.after=1)%>%
  mutate(across(4:last_col(), ~ as.numeric(.)))%>%
  pivot_longer(cols=-c("code","Region","Header"),names_to = "Date",values_to ="value")%>%
  mutate(table="7dtab",Date=ymd(Date),
         forecast=ifelse(Date>=file_date,1,0),
         version=file_date)%>%
  I()
}
...
#stack everthing
  steo_data<-rbind(price_outlook,crude_supply_data,non_opec_supply_data,opec_supply_data,nat_gas_data,power_data)
steo_data
}


steo_data0<-filter(steo_data_fetch(steo_date),Date>=ymd("2023-1-01"),forecast==0)
get_history<-0
if(get_history==1)
  {
  #2023-2019 history
  steo_data5<-filter(steo_data_fetch(ymd("2023-1-1")),Date>=ymd("2019-01-01"),forecast==0)
  #2015-2018 history
  steo_data4<-filter(steo_data_fetch(ymd("2019-1-1")),Date>=ymd("2015-01-01"),forecast==0)
  #2011-2014 histories
  steo_data3<-filter(steo_data_fetch(ymd("2015-1-1")),forecast==0)
  #2007-2010 histories
  steo_data2<-filter(steo_data_fetch(ymd("2011-1-1")),forecast==0)
  #2004-2007 histories
  steo_data1<-filter(steo_data_fetch(ymd("2008-1-1")),Date<ymd("2007-01-01"),forecast==0)
  steo_history<-bind_rows(steo_data1,steo_data2,steo_data3,steo_data4,steo_data5)
  save(steo_history,file="steo_history.RData")
  }

load("steo_history.RData")
steo_history<-bind_rows(steo_history,steo_data0)
...
steo_old_sd_forecasts<-filter(steo_data_fetch(ymd("2019-12-1")),Date>=ymd("2015-01-01"),forecast==1) %>%
  ...
  rbind(filter(steo_data_fetch(ymd("2023-12-1")),Date>=ymd("2015-01-01"),forecast==1))%>%
...
graph_df<-supply_demand%>%
  mutate(Region=as_factor(Region),
         Region=ifelse(code=="papr_world","Total World Supply",Region),
         Region=ifelse(code=="patc_world","Total World Consumption",Region),
...
demand<-
  ...
  geom_point(data=filter(graph_df,Region=="Total World Consumption",forecast==1),
             aes(Date,value,group=version,shape=version,colour=version),size=2.5)+
  ...
  labs(y="Global Liquids Demand (million barrels per day)",x="",
       title=paste("Global Liquids Demand"),
       subtitle=paste("Historic Values and EIA Short Term Energy Outlook Forecasts"),
...
ggsave("images/demand.png",width=13,height = 7,dpi=res,bg="white")
...
ggsave("images/demand_plain.png",width=16,height = 10,dpi=300,bg="white")
  ...
  get_old_wti<-function(){
      ...
      bind_rows(filter(steo_data_fetch(ymd("2023-1-1")),forecast==1))%>%
      bind_rows(filter(steo_data_fetch(ymd("2025-1-1")),forecast==1))%>%
      bind_rows(filter(steo_data_fetch(ymd("2026-1-1")),forecast==1))%>%
  ...
  budget_2023 <- data.frame("Date" = c("2021-10-01","2022-10-01","2023-10-01","2024-10-01","2025-10-01"), 
                            "WTI_CAD" = c(96.5,120,104,97.20,92.50),
                            "WTI" = c(77.03,90.50,79,76,73.50),stringsAsFactors = F)
  ...
  budget_2023$Date<-ymd(budget_2023$Date)
  ...
  budget_2023$version<-"Alberta Budget 2023"
  budget_2023$version<-factor(budget_2023$version)
  ...
  budget_2023 <- budget_2023 %>% rename("value"="WTI") %>% select(-WTI_CAD) %>%filter(Date>=ymd("2023-02-01"))%>%
    mutate(forecast=3)
  ...
  wti_graph+
    ...
    geom_point(aes(ymd("2023-02-25"),92.1),size=4,color="black")+
    annotate("segment",x=ymd("2023-02-25"),y=92.1,xend=ymd("2023-02-25"),yend=92.10+50)+
    geom_line(data=budget_2021,aes(Date,value,group=version,linetype="Z2"),size=1.15,color="black")+
    geom_label(aes(x=ymd("2023-02-24"),y=92.1),nudge_y=50,size=4,label="Budget Day 2023")+
    geom_line(data=budget_2022,aes(Date,value,group=version,linetype="Z3"),size=1.15,color="black")+
    geom_line(data=budget_2023,aes(Date,value,group=version,linetype="Z4"),size=1.15,color="black")+
    scale_x_date(breaks = "24 months",date_labels = "%Y",expand=c(0,0))+
    expand_limits(x=ymd("2025-01-01"))+
    scale_linetype_manual("",values=c("solid","11","31","33","52"),labels=c("Historic WTI Monthly Prices","Alberta Budget 2020","Alberta Budget 2021","Alberta Budget 2022","Alberta Budget 2023"))+
