...
steo_date <- html_table(page, fill = TRUE)[[1]]%>%
  clean_names()%>%
...
clean_sheet<-function(file_sent=excel_file,sheet_name="2tab",range_sent = "A4:BV40",date_sent=file_date,year_sent=year_start,month_sent=month_start,na_sent="n/a",names_sent=c("code","Region"),drop_rows=NA){
  names_use<-c("code",names_sent,format(seq.Date(from=ymd(paste(year_sent,month_sent,1,sep="-")),by="1 month",length.out=72)))
  data_read<-read_excel(path=file_sent,sheet = sheet_name,range = range_sent,na=na_sent,skip=0,col_names=FALSE)
  names(data_read)<-names_use
  #data_read<-
  data_read%>%
    slice(-1)%>% #remove the date row
    #filter(!is.na(code))%>%
    filter(!row_number()%in% drop_rows)%>%
    mutate(cat=ifelse(is.na(code),.[[2]],NA))%>%
    fill(cat,.direction=c("down"))%>%
    filter(!is.na(code))%>%
    relocate(cat,.after=NROW(names_sent)+1)%>%
    pivot_longer(cols=-c(code,commodity,cat),names_to = "date",values_to = "value")%>%
    mutate(table=sheet_name,
           date=ymd(date),
           version=date_sent,
           forecast=ifelse(date>=version,TRUE,FALSE),
    )%>%
    I()
}
...
#stack all the data
#find the earliest date in the file
...
price_outlook<-clean_sheet(file_sent=excel_file,sheet_name= "2tab",range_sent = "A5:BV40",drop_rows = c(7,13,15,19),names_sent = c("commodity"))
crude_supply<-clean_sheet(file_sent=excel_file,sheet_name= "3atab",range_sent = "A5:BV46",drop_rows = c(1),names_sent = c("commodity"))
...
us_gas_supply<-read_excel(path=excel_file,sheet = "5atab",range = "A4:BV55",na=c("n/a","-"))



test<-clean_sheet(drop_rows = c(7,13,15,19),names_sent = c("commodity"))
